...
def cross_entropy_loss(out, y):
    ...
    log_out = minitorch.nn.logsoftmax(out, dim=0)
    log_one_minus_out = minitorch.nn.logsoftmax(ones - out, dim=0)

    loss = -(y * log_out + (ones - y) * log_one_minus_out)
...
class Linear(minitorch.Module):
    ...
    def forward(self, x):
        ...
        weights_reshaped = self.weight.value.view(in_size, self.out_size)
        output = x_reshaped @ weights_reshaped
...
class Network(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        averaged = embeddings.mean(dim=1).view(batch, embedding_dim)

        hidden = self.linear1(averaged)
        hidden = hidden.relu()
        hidden = minitorch.dropout(hidden, self.dropout_prob, ignore=not self.training)
        output = self.linear2(hidden)
        output = output.sigmoid()
        return output.view(batch)
    ...
    def train(
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            if data_val is not None:
                ...
                y_val_tensor = minitorch.tensor(y_val, backend=BACKEND, requires_grad=False)

                val_out = model(x_val)

                validation_predictions = get_predictions_array(y_val, val_out)
