...
__global__ void mapKernel(
   ...
   */
    ...
    float input_value = in_storage[in_position];
    float output_value = fn(fn_id, input_value);
    out[out_position] = output_value;
...
__global__ void zipKernel(
   ...
   */
    ...
    float a_value = a_storage[a_position];
    float b_value = b_storage[b_position];
    float result = fn(fn_id, a_value, b_value);
...
__global__ void reduceKernel(
   ...
   */
    ...
    __shared__ float cache[BLOCK_DIM]; // Uncomment this line if you want to use shared memory to store partial results
    int out_index[MAX_DIMS];
    ...
    for (int i = tid; i < a_shape[reduce_dim]; i += block_size) {
      in_index[reduce_dim] = i;
      int in_pos = index_to_position(in_index, a_strides, shape_size);
      float val = a_storage[in_pos];
      acc = fn(fn_id, acc, val);
    }
    cache[tid] = acc;
    __syncthreads();
...
__global__ void MatrixMultiplyKernel(
   ...
   */
    ...
    for (int t = 0; t < num_tiles; ++t) {
      if (row < m && (t * TILE + ty) < n) {
        a_shared[tx][ty] = a_storage[batch * a_batch_stride + row * a_strides[1] + (t * TILE + ty) * a_strides[2]];
      } else {
        a_shared[tx][ty] = 0;
      }
      if((t * TILE + tx) < n && col < p) {
        b_shared[tx][ty] = b_storage[batch * b_batch_stride + (t * TILE + tx) * b_strides[1] + col * b_strides[2]];
      } else {
        b_shared[tx][ty] = 0;
      }
      __syncthreads();
