...
class GPT2ModelParallel(GPT2ModelCustom):
    ...
    def _prepare_pipeline_parallel(self, split_size=1):
        ...
        # BEGIN ASSIGN5_2_3
        # Enable pipeline parallel mode
        self.pipeline_parallel = True
        
        # Create a custom wrapper class for GPT2Block that only returns hidden states
        ...
        # Create a sequential module with wrapped transformer blocks
        sequential_module = nn.Sequential(*[GPT2BlockWrapper(block) for block in self.h])
        
        # Create the pipeline
        pipe = Pipe(sequential_module, split_size=split_size)
        # END ASSIGN5_2_3
        self.h_pp = pipe
