...
class Embedding(Module):
    ...
    def forward(self, x: Tensor):
        ...
        # COPY FROM ASSIGN2_3
        one_hot_vectors = one_hot(x.view(bs * seq_len), self.num_embeddings)
        embeddings = one_hot_vectors @ self.weights.value
...
class Dropout(Module):
    ...
    def forward(self, x: Tensor) -> Tensor: 
        ...
        """
        # COPY FROM ASSIGN2_3
        if self.training and self.p_dropout:
            mask = tensor_from_numpy(np.random.binomial(1, 1-self.p_dropout, x.shape).astype(np.float32), backend = x.backend)
