...
class Attn_Softmax(Function):
    ...
    def backward(ctx: Context, out_grad: Tensor) -> Tensor:
      ...
      grad_result = out_grad.f.attn_softmax_bw(out_grad, soft_inp)
      return grad_result, out_grad.zeros(out_grad.shape)
      #   END ASSIGN4_1_2
...
class LayerNorm(Function):
    ...
    def forward(ctx: Context, inp: Tensor, gamma: Tensor, beta: Tensor) -> Tensor:
      #   BEGIN ASSIGN4_2_1
      ln_res, mean, var = inp.f.layernorm_fw(inp, gamma, beta)
      ctx.save_for_backward(ln_res, inp, var, mean, gamma, beta)
      return ln_res
      #   END ASSIGN4_2_1
    ...
    def backward(ctx: Context, out_grad: Tensor) -> Tensor:
      #   BEGIN ASSIGN4_2_2
      (ln_res, inp, var, mean, gamma, beta) = ctx.saved_values
      inp_grad, gamma_grad, beta_grad = ln_res.f.layernorm_bw(out_grad, inp, gamma, beta, var, mean)
