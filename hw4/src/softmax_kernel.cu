...
void launch_attn_softmax_bw(float *out_grad,
  ...
  cudaMalloc((void **)&d_soft_inp, inp_size);

  cudaMemcpy(d_out_grad, out_grad, grad_size, cudaMemcpyHostToDevice);
  cudaMemcpy(d_soft_inp, soft_inp, inp_size, cudaMemcpyHostToDevice);
  ...
  if (softmax_len <= 32) {
    ...
    throw std::runtime_error("Sequence length greater than 1024 is currently not supported");
  }
  // Copy back to the host
