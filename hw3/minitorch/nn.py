...
def logsumexp(input: Tensor, dim: int) -> Tensor:
    ...
    """  
    ### BEGIN ASSIGN3_1
    max_x = Max.apply(input, tensor([dim]))
    exp_term = (input - max_x).exp()
    sum_exp = exp_term.sum(dim=dim)
    return sum_exp.log() + max_x
    ### END ASSIGN3_1
...
def softmax_loss(logits: Tensor, target: Tensor) -> Tensor:
    ...
    log_softmax_result = logsoftmax(logits, dim=1)
    target_one_hot = one_hot(target, num_classes)
    result = -(target_one_hot * log_softmax_result).sum(dim=1)
    ### END ASSIGN3_1
